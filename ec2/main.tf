...
resource "aws_ssm_patch_baseline" "al_patch_baseline" {
...
resource "aws_ssm_patch_group" "al_patch_group" {
  baseline_id = aws_ssm_patch_baseline.al_patch_baseline.id
  patch_group = "AmazonLinuxPatchGroup"
}

resource "aws_ssm_patch_group" "al2022_patch_group" {
  baseline_id = aws_ssm_patch_baseline.al2022_patch_baseline.id
  patch_group = "AmazonLinux2022PatchGroup"
}
...
resource "aws_ssm_maintenance_window" "example" {
  ...
  schedule                 = "cron(0 10 * * ? *)" # This is the cron expression for 10:00 AM CST
